"""Optional smoke test that hits the real Ashby Job Postings API.
...
BOARD = "openai"  # a real, currently-active Ashby board as of investigation
...
def test_live_search_returns_well_formed_jobs():
    with AshbySource(BOARD) as source:
        ...
        for raw in raw_jobs:
            ...
            assert job.title
            assert job.source_url.startswith("https://jobs.ashbyhq.com/")
...
def test_live_unknown_board_raises_not_found():
    from careers_os.sources.ashby.client import AshbyBoardNotFoundError

    with AshbySource("this-board-should-not-exist-12345") as source:
